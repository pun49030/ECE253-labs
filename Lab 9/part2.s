...
PRIV_TIMER_CHECK:
    	//		... [FILL IN CODE HERE ]..........................
		CMP R5, #MPCORE_PRIV_TIMER_IRQ // 29 if interval timer interrupt ID
		BNE KEYS_CHECK // if interrupt not from timer
		BL PRIV_TIMER_ISR
		B EXIT_IRQ
...
KEYS_CHECK:
		...
		CMP R5, #KEYS_IRQ // 73 is key interrupt ID
		// BNE EXIT_IRQ
		BL KEY_ISR
...
SWEEP:		LDR		R0, =LEDR_DIRECTION	// put shifting direction into R2
				...
				CMP R2, #1 // check if direction is outwards
				PUSH {R10-R11}
				BEQ TOOUTSIDE
				
TOCENTRE:		
				LDR R10, =#0b1000000001
				CMP R3, R10 // check if at ends
				LDR R11, =#0b0100000010
				MOVEQ R3, R11
				BEQ DONE_SWEEP
				
				LDR R10, =#0b0100000010
				CMP R3, R10 
				LDR R11, =#0b0010000100
				MOVEQ R3, R11
				BEQ DONE_SWEEP
				
				LDR R10, =#0b0010000100
				CMP R3, R10 
				LDR R11, =#0b0001001000
				MOVEQ R3, R11
				BEQ DONE_SWEEP
				
				LDR R10, =#0b0001001000
				CMP R3, R10 
				LDR R11, =#0b0000110000
				MOVEQ R3, R11
...
_start:
			...
			// setup stack pointers for modes
...
CONFIG_PRIV_TIMER:
			...
			LDR R1, =50000000
			STR R1, [R0] // store timer value
			
			MOV R1, #0b111
			STR R1, [R0, #8] // enable interrupt for TIMER
			MOV 		PC, LR 					// return
...
CONFIG_KEYS:
			...
			MOV R1, #0b1000
			STR R1, [R0, #8] // enable interrupt for KEY3 using interrupt mask register
